...
// because api() is a plain function called from every page.
...
// which is not.
const FALLBACK = {
  404: "Can't reach the server. The site may need updating",
...
export async function api(path, { method = "GET", body } = {}) {
  if (!BASE_URL) {
    throw new Error("VITE_API_URL is not set. See web/.env.example");
  }
  ...
  if (!res.ok) {
    ...
    // it has expired or the account is gone. Drop it and say so once. Gated
    // on `token` so /login answering 401 to a wrong password never trips it.
